const pHayComprobante = document.getElementById('hay-comprobante')
if (pHayComprobante)
    pHayComprobante.style.display = 'none'

const pNoHayComprobante = document.getElementById('no-hay-comprobante')
if (pNoHayComprobante)
    pNoHayComprobante.style.display = 'none'

const inputComprobante = document.getElementById('mostrar-comprobante')
if (inputComprobante)
    inputComprobante.style.display = 'none'
...
if (modalEditar) {
    modalEditar.addEventListener('close', evt => {
        if (pHayComprobante)
            pHayComprobante.style.display = 'none'
        if (pNoHayComprobante)
            pNoHayComprobante.style.display = 'none'
    ...
    botonesEditar.forEach(boton => {
        boton.addEventListener('click', evt => {
            if (modalEditar) {
                ...
                document.getElementById('id_fecha_cita').value = boton.dataset.fecha
                ...
                setTimeout(() => {
                    ...
                    picker.on('changeMonth', evt => {
                        ...
                        */
                        setTimeout(() => {
                            // let fecha = picker.datepicker('getFormattedDate')
                            // if (fecha === null) {
                            //     // fecha = document.getElementById('id_fecha_cita').value 
                            // }
...
async function getHorarios(year, month, day) {
    try {
        ...
        if (element) {
            ...
            json.forEach(d => {
                let template = `<option value="${d[0]}">${d[1]}</option>`
                new_ele = document.createElement('div')
                new_ele.innerHTML = template
                element.insertAdjacentElement('beforeend', new_ele)
            })
    ...
    fetch(`/api/fechas-bloqueadas/${year}/${month}?format=json`, {
        ...
        .then(data => {
            if (data.length > 0) {
                ...
                data.forEach(obj => {
                    if (obj.dia_entero_bloqueado) {
                        disabled_dates.push(new Date(obj.unix_timestamp))
                    }
                })
